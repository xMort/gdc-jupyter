...
class PredictionAnalyzer(InsightAnalyzer):
    ...
    def predict_bounds(self, confidence_interval=0.95, steps=50):
        ...
        model = SARIMAX(
            interpolated_series, order=(1, 1, 1), seasonal_order=(1, 1, 1, 12)
        )
...
class InsightAnalyzer1:
    """ Unused, this is mainly for testing in comparison to the working one (e.g. different predictors, clusters..)"""
    def __init__(self, insight_id: str, workspace: str | None = None):
        load_dotenv()
        self.host = os.getenv("HOST")
        self.token = os.getenv("TOKEN")
